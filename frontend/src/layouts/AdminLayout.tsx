...
function initials(name?: string) {
  return (name || "Administrator").trim().split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join("").toUpperCase();
}
...
export default function AdminLayout() {
  ...
  return <div className="min-h-screen bg-slate-50 text-foreground">
    ...
    <main className="min-h-screen md:ml-64">
      <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/90 backdrop-blur"><div className="flex h-14 items-center gap-2 px-4 sm:h-16 sm:px-5 md:px-8"><Menu className="size-5 text-primary md:hidden" /><span className="text-sm font-medium text-muted-foreground">Admin workspace</span><button onClick={() => { logout(); navigate("/"); }} className="ml-auto text-sm font-medium text-red-600 md:hidden">Sign out</button></div><nav className="flex gap-1 overflow-x-auto border-t border-slate-100 px-3 py-2 md:hidden">{links.map(({ to, label, icon: Icon }) => <NavLink key={to} to={to} className={navClass}><Icon className="size-4" />{label}</NavLink>)}</nav></header>
      <div className="p-4 sm:p-5 md:p-8"><Outlet /></div>
    </main>
  </div>;
}
